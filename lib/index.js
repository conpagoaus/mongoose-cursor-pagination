...
module.exports = (schema, { defaultLimit = 100 } = {}) => {
  ...
  const newExec = async function (_origExec, ...args) {
    ...
    let startCursor = null,
      nextCursor = null;
    const firstCursorObj = createCursorObj(results[0], sort);
    startCursor = generateCursorStr(firstCursorObj);
    if (hasNextPage) {
      results.pop();
      const lastResult = results[results.length - 1];
      const nextCursorObj = createCursorObj(lastResult, sort);

      nextCursor = generateCursorStr(nextCursorObj);
    }

    results = results.map((v) => ({
      cursor: generateCursorStr(v),
      node: v,
    }));
    ...
    return {
      ...
      results,
      pageInfo: { hasNextPage, startCursor, nextCursor },
    };
  };
