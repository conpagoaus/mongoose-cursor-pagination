...
describe('utils#generateCursorStr', () => {
  ...
  it('Works with cursor data with multiple properties', () => {
    const cursorObj = {
      _id: '123',
      name: "Jane D'Souza & Co.",
      timestamp: 1550169401498,
    };
    const cursorStr = generateCursorStr(cursorObj);
    const str =
      '{"_id":"123","name":"Jane D\'Souza & Co.","timestamp":1550169401498}';
    expect(cursorStr).toBe(base64url.encode(str));
  });

  it('Is able to serialize Object Ids as strings', () => {
    const cursorObj = {
      _id: new mongoose.Types.ObjectId('5b06b90b42a0b29ba10f20c2'),
    };
    const cursorStr = generateCursorStr(cursorObj);
    expect(cursorStr).toBe(
      base64url.encode('{"_id":"5b06b90b42a0b29ba10f20c2"}')
    );
  });
